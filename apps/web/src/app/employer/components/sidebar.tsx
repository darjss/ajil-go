...
function NavBlock({ items, title }: { items: typeof employerNav; title?: string }) {
...
export function EmployerSidebar() {
	return (
		<aside className="hidden lg:flex w-72 bg-white border-r border-border/70 flex-col justify-between">
			<div>
				<div className="p-6">
					<h1 className="text-xl font-bold text-foreground mb-8">JobHuntly</h1>
			...
			<div className="p-6 border-t border-border/70">
				<div className="flex items-center gap-3">
					...
					<div className="flex-1 min-w-0">
						<p className="text-sm font-semibold text-foreground truncate">{employerProfile.name}</p>
						<p className="text-xs text-muted-foreground truncate">{employerProfile.email}</p>
					</div>
				</div>
			</div>
		</aside>
